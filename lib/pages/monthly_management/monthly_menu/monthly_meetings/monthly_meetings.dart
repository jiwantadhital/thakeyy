...
import 'package:thadri/pages/monthly_management/monthly_menu/monthly_meetings/monthly_bottom_sheet.dart';
import 'package:thadri/pages/text/text.dart';

class MonthlyMeeting extends StatefulWidget {
  const MonthlyMeeting({Key? key}) : super(key: key);

  @override
  State<MonthlyMeeting> createState() => _MonthlyMeetingState();
}

class _MonthlyMeetingState extends State<MonthlyMeeting> {
  @override
  Widget build(BuildContext context) {
    var pageId=Get.parameters['pageId'];
    return Scaffold(
      appBar: AppBar(
        title: Center(child: Text("मासिक वैठक")),
      ...
      body: Container(
        ...
        child: Padding(
          ...
          child: GetBuilder<MonthlyMeetingsController>(builder: (group){
                return group.isLoaded?ListView.builder(
                  itemCount: group.monthlyMeetingList.length,
            ...
            return GestureDetector(
              onTap: (){
               showModalBottomSheet(
                 ...
                 return MonthlyBottomSheet(pageId: index);
               });
              },
              child: Card(
                color: Colors.grey.shade300,
                    child: Padding(
                      ...
                      child: ListTile(
                        title: Text(
                          group.monthlyMeetingList[index].samuhaName!.samuhakoName.toString(),
                          style: TextStyle(
                            fontSize: 20,
                          ),
                        ),
                        leading: CircleAvatar(
                          child: Text(
                            group.monthlyMeetingList[index].id.toString(),
                            style:
